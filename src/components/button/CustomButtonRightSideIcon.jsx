...
export const CustomButtonRightSideIcon = styled(Button)(({ theme, responsive, variant, size }) => {
  let fontSize;
  let padding;
  ...
  switch (size) {
    ...
    case 'medium':
      ...
      padding = responsive.isMobile ? '6px 12px' : responsive.isTablet ? '8px 16px' : responsive.isDesktop ? '10px 20px' : '12px 24px';
      break;
    case 'small':
...
export const CustomInputButtonWithIcon = styled(CustomButtonRightSideIcon)`
  display: flex,
  align-items: center,
...
const YourComponent = ({ porps }) => {
  const responsive = useResponsiveStyles();

  return (
    <div>
      <CustomInputButtonWithIcon variant="contained" color="primary" responsive={responsive}>
        Button Text
        {/* <IconButton>
          <ArrowForwardIcon />
        </IconButton> */}
      </CustomInputButtonWithIcon>
    </div>
  );
};
